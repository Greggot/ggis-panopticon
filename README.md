...
Для корректной работы приложения необходимо установить расширения. Для этого вызовите следующую команду
```bash
...
Оба скрипта `./skird_cmd.py` и `./skird.py` позволяют работать через интерактивный интерфейс. Для этого необходимо передать аргумент `--tui` при вызове скрипта 
